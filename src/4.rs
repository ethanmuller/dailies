...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    model.oscillator = ((elapsed_secs * std::f32::consts::PI * model.oscillator_freq_mult).sin()*model.oscillator_freq_mult)*model.oscillator_amp;
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    if frame.nth() == 0 || app.keys.down.contains(&Key::Back) {
        draw.background().rgba(0.0, 0.0, 0.0, 1.0);
    } else {
        let alpha = model.oscillator.abs()*model.overlay_alpha;
        draw.rect()
